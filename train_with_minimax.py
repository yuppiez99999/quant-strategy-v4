# -*- coding: utf-8 -*-
"""
使用MiniMax-M3模型训练量化策略
结合LLM进行宏观分析、因子挖掘和策略优化
"""
...
class MiniMaxQuantTrainer:
    ...
    def load_model(self):
        print(f"\n📥 加载MiniMax-M3模型: {self.model_path}")
        try:
            ...
            print("✅ MiniMax-M3模型加载成功")
            return True
        except Exception as e:
            print(f"❌ 模型加载失败: {e}")
            return False
    
    def analyze_with_llm(self, market_data, query):
        """使用MiniMax-M3进行市场分析"""
        if not self.model:
            return "模型未加载"
        
        prompt = f"""
你是一位资深的量化投资分析师，请基于以下市场数据进行分析：
...
"""
    ...
    def generate_signal(self):
        ...
        for code in self.codes:
            ...
            # 买入信号: RSI < 30 且价格接近布林带下轨
            buy_condition = (code_data['rsi_14d'] < 30) & (code_data['bb_position'] < 0.2)
            code_data.loc[buy_condition, 'signal'] = 1
            
            # 卖出信号: RSI > 70 且价格接近布林带上轨
            sell_condition = (code_data['rsi_14d'] > 70) & (code_data['bb_position'] > 0.8)
    ...
    def backtest_with_signals(self, initial_capital=1000000):
        ...
        for date in dates_sorted:
            ...
            # 执行信号 (itertuples 比 iterrows 快 2-3 倍)
    ...
    def run(self, excel_path, use_llm_analysis=True):
        """运行完整训练流程"""
        print("""
╔═══════════════════════════════════════════════════════════╗
║         MiniMax-M3 量化策略训练系统                      ║
...
╚═══════════════════════════════════════════════════════════╝
        ...
        if use_llm_analysis:
            if self.load_model():
                market_summary = f"""
数据覆盖: {self.df['日期'].min().strftime('%Y-%m-%d')} 至 {self.df['日期'].max().strftime('%Y-%m-%d')}
标的数量: {len(self.codes)} 只
总记录数: {len(self.df)} 条

回测结果:
- 年化收益率: {self.metrics['annualized_return']*100:.2f}%
- 最大回撤: {self.metrics['max_drawdown']*100:.2f}%
- 夏普比率: {self.metrics['sharpe_ratio']:.2f}
                """
                
                print("\n🤖 LLM分析中...")
                analysis = self.analyze_with_llm(market_summary, "请分析当前量化策略的表现，指出优势和改进空间，并提供优化建议")
                print("\n" + "="*70)
                print("🤖 MiniMax-M3 分析报告")
                print("="*70)
                print(analysis)
...
def main():
    ...
    model_path = r'e:\各种PY程序\13_超算业务系统\models\MiniMax-M3'
    ...
    trainer = MiniMaxQuantTrainer(model_path)
